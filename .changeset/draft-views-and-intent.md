...
Draft `kdiagram 2` views and intent: optional `intent { }`, shared `model` + embedded `view` lenses, `--view` targeting, `analyze --compare-layouts`, studio/embed view switchers, gallery entry for `storefront-model`, and architecture notes in `docs/architecture/views-and-intent.md`. One self-contained file per model — no cross-file `import`. Breaking changes remain gated on `kdiagram 2`; `diagram { }` stays the default path.

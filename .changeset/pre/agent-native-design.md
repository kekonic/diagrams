...
"@kekonic/diagrams-layout": patch
"@kekonic/diagrams-language-service": patch
"@kekonic/diagrams": patch
"@kekonic/diagrams-render-svg": patch
"@kekonic/diagrams-ui": patch
"@kekonic/diagrams-website": minor
---

Add the official host-neutral `design-kekonic-diagrams` agent skill with grounded modeling,
...
initial/final rendering. Add post-layout quality diagnostics for extreme canvases,
...
Add a versioned built-in capability manifest through the JavaScript API and `kdiagrams capabilities`,
plus `kdiagrams analyze` batch JSON output with rendered dimensions, algorithm provenance, and
deterministic quality diagnostics for agent and CI repair loops.
Define future agent patterns as canonical worked exemplars—realistic briefs, excellent standalone
source, reviewed artifacts, rationale, failure modes, and repeatable validation—rather than typed
decision logic. Keep agent guidance out of the human CLI surface.
Rewrite the public skill and its focused references in plain language rooted in ordinary software
work. Use direct questions about what something is, what it does, who owns it, and how it is known.
Add the first complete reference example: a realistic order-fulfillment prompt, the material
questions and answers, reviewable design decisions, rejected layout alternatives, final KDiagram
source, two purposeful animation stories, inspected portable SVG, and measured final review.

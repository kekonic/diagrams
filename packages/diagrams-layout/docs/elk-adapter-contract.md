...
| `groupLayout: swimlane` or `kind: swimlane`        | Flattened leaves so ELK ranks a **shared timeline** in the author direction. Rework edges that close a cycle are omitted from ranking (still routed after). `elk.layered.layering.layerChoiceConstraint` pins each node to its process rank. `applySwimlaneBands` then packs members into declaration-order bands (no gap), shares width, and paints the left header strip. Nested lane compounds are not used: they ranked each lane in isolation and inverted process time. |

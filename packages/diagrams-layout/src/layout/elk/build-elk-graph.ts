...
import { resolveSwimlaneLayoutOptions } from "../swimlane-bands.ts";
...
export function buildElkGraph(
  ...
  const useSwimlaneTimeline = topSwimlanes.length > 0 && options.groupLayout !== "flat";
  ...
  // Swimlanes also flatten: ELK ranks a shared LR timeline, then applySwimlaneBands packs
  // Y. Nested lane compounds ranked each lane in isolation and put process time backwards.
  const flatMembership =
    options.groupLayout === "flat" || useSwimlaneTimeline || options.groupLayout !== "compound";
  ...
  const timeline = useSwimlaneTimeline ? swimlaneProcessRanks(graph) : undefined;
  ...
  const rootLayoutOptions: Record<string, string> = {
    ...
    "elk.layered.considerModelOrder.strategy":
      useSwimlaneTimeline || options.considerModelOrder === false ? "NONE" : "NODES_AND_EDGES",
  ...
  for (const g of graph.groups) {
    // Flat: do not nest compound ELK nodes; membership boxes are painted later.
  ...
  const nodeLayoutOptions = (nodeId: string): Record<string, string> => {
    ...
    const layer = timeline?.rank.get(nodeId);
    if (layer != null) {
      opts["elk.layered.layering.layerChoiceConstraint"] = String(layer);
    } else {
  ...
  const edges = graph.edges
    .filter((e) => !timeline?.feedbackEdgeIds.has(e.id))
    .map((e) => ({
      id: e.id,
      sources: [sourceRef(e)],
      targets: [targetRef(e)],

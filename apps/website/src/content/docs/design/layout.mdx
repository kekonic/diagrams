...
Visible groups are a dashed outline without a fill — nested boxes would otherwise stack
...
For ports-and-adapters or onion-style views, nest a hub group and put satellite **nodes** as
...
Satellite nodes sit in the annulus **inside** the parent chrome and **outside** the hub. Depth is
unlimited: the hub may itself use `arrange: surround`. KDiagram does not attach ports to hex faces.
...
<KDiagramPlayground source={orderFulfillment} height={620} editorHeight={450} />
...
- `metro` (default) and `rounded` — orthogonal corridors with eased ports and
  curved avoidance (not sharp 90° stairs)
- `orthogonal` — the same corridors with sharp corners
- `straight` — a true port-to-port chord when that line misses other nodes; otherwise a
  short dogleg that follows the existing corridor
- `bezier` — a smoother cubic along that corridor, using port exit/entry directions and
  splitting around nodes instead of drawing a naive center-to-center S-curve
...
`swimlane` groups are horizontal responsibility bands with a shared left-to-right process
...
[Expense approval](/gallery/expense-approval/).

`arrange: row` (or stacked groups inside a row) is the right tool for **owner columns** — architecture
or workflow bands that do not share a process axis. See [Customer refund request](/gallery/refund-request/).

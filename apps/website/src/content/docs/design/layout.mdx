...
description: Guide Kekonic Diagrams with direction, density, groups, region arrangement, and edge treatment—without storing coordinates.
---
...
<div class="kd-lead">
  Kekonic Diagrams owns coordinates, but you own visual structure. Give the engine a reading
  direction, meaningful boundaries, and a small number of constraints. Tune the result only after
  the model is clear.
</div>
...
Kekonic Diagrams lays out the contents of a group and the relationships between groups. That makes a group
...
Kekonic Diagrams does not expose arbitrary coordinates. If the exact position of an individual shape is a

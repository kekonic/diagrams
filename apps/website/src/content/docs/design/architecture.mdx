...
`kdiagram 2` **model** with embedded **views** when context and containers share topology — then
switch lenses with `--view` or the live host picker instead of copying files:
...
- [Storefront — shared model with views](/gallery/storefront-model/) (context + containers from one source)
- Standalone twins remain for teaching single-file diagrams:
  [context](/gallery/storefront-context/),
  [containers](/gallery/storefront-containers/),
  [components](/gallery/storefront-components/)
...
when the model is right but the visual hierarchy needs help, or browse the [architecture gallery](/gallery/order-fulfillment/).

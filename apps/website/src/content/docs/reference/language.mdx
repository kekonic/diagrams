...
State diagrams support `initial`, `state`, `final`, `junction`, `fork`, and `join`. KDiagram checks
that there is exactly one initial pseudostate, initial has no incoming transitions, and final states
...
Simple diagrams should stay as a single `diagram { … }` — no modeling ceremony.
...
model "Storefront" {
  ...
  view context {
    intent {
      ...
      question: "Who uses the platform and which systems sit outside it?"
      omits: "Internal containers and databases"
    }
...
| Construct                                          | Role                                                                                          |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `model "Title" { … }`                              | Shared semantic topology (nodes, edges, groups). No layout/presentation at this layer.        |
| `view name { … }`                                  | Named lens: scope, layout, presentation, optional intent.                                     |
| `include` / `exclude`                              | Selectors: bare id, `prefix.*` (group subtree or id prefix), or `*`.                          |
| `collapse group as id: kind "Label" { … }`         | Replace a group subtree with one summary node. Optional `description` / `technology`.         |
| `intent { … }`                                     | Editorial metadata (`audience`, `question`, `scope`, `omits`, `assumptions`, `evidence`). Not drawn in SVG. |
...
Use `layout { groupLayout: swimlane }` when top-level swimlanes represent responsibility.
...
source policy; source policy overrides built-in defaults.

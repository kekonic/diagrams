...
<div class="kd-lead">
  The <code>kdiagram</code> command validates source, renders SVG, formats files, and exposes parsed
...
Use `pnpm dlx @kekonic/diagrams-cli …` for a one-off evaluation. Use a project-local dependency
for builds and CI.
...
software, desktop Markdown readers, and other hosts without KDiagram CSS. `--live-theme` prints a
...
For a `kdiagram 2` model with multiple `view` blocks, add `--compare-layouts` to render every view
and score shared-node layout stability (normalized centers). Stability below `0.55` emits
`view-layout-instability`. Deliberate TD-versus-LR pairs often warn — that is expected when shared
actors sit on different axes.
...
`--view` also applies to `render`, `check`, `graph`, and `ast` when the input is a model.
